...
CREATE INDEX
    idx_captcha_expired_on ON p_captcha USING btree (expired_on);

CREATE INDEX
    idx_captcha_use_times ON p_captcha USING btree (use_times);
...
CREATE INDEX
    idx_comment_content_comment_id ON p_comment_content USING btree (comment_id);

CREATE INDEX
    idx_comment_content_user_id ON p_comment_content USING btree (user_id);

CREATE INDEX
    idx_comment_content_type ON p_comment_content USING btree ("type");

CREATE INDEX
    idx_comment_content_sort ON p_comment_content USING btree (sort);
...
CREATE INDEX
    idx_comment_reply_comment_id ON p_comment_reply USING btree (comment_id);
...
CREATE INDEX
    idx_comment_metric_comment_id_rank_score ON p_comment_metric USING btree (comment_id, rank_score);
...
CREATE TABLE
    p_tweet_comment_thumbs (
        ...
        -- 评论类型 0为推文评论、1为评论回复
        is_thumbs_up SMALLINT NOT NULL DEFAULT 0,
        -- 是否点赞 0 为否 1为是
        is_thumbs_down SMALLINT NOT NULL DEFAULT 0,
        -- 是否点踩 0 为否 1为是
        created_on BIGINT NOT NULL DEFAULT 0,
        modified_on BIGINT NOT NULL DEFAULT 0,
        deleted_on BIGINT NOT NULL DEFAULT 0,
...
CREATE INDEX
    idx_tweet_comment_thumbs_uid_tid ON p_tweet_comment_thumbs USING btree (user_id, tweet_id);
...
CREATE INDEX
    idx_message_receiver_user_id ON p_message USING btree (receiver_user_id);
...
CREATE TABLE
    p_post (
        ...
        is_essence SMALLINT NOT NULL DEFAULT 0,
        -- 是否精华
        is_lock SMALLINT NOT NULL DEFAULT 0,
        -- 是否锁定
...
CREATE INDEX
    idx_post_metric_post_id_rank_score ON p_post_metric USING btree (post_id, rank_score);
...
CREATE INDEX
    idx_post_attachment_bill_post_id ON p_post_attachment_bill USING btree (post_id);

CREATE INDEX
    idx_post_attachment_bill_user_id ON p_post_attachment_bill USING btree (user_id);
...
CREATE INDEX
    idx_post_collection_post_id ON p_post_collection USING btree (post_id);

CREATE INDEX
    idx_post_collection_user_id ON p_post_collection USING btree (user_id);
...
CREATE INDEX
    idx_post_content_post_id ON p_post_content USING btree (post_id);

CREATE INDEX
    idx_post_content_user_id ON p_post_content USING btree (user_id);
...
CREATE INDEX
    idx_post_star_post_id ON p_post_star USING btree (post_id);

CREATE INDEX
    idx_post_star_user_id ON p_post_star USING btree (user_id);
...
CREATE INDEX
    idx_user_metric_user_id_tweets_count_trends ON p_user_metric USING btree (
        user_id,
        tweets_count,
        latest_trends_on
    );
...
CREATE INDEX
    idx_following_user_follow ON p_following USING btree (user_id, follow_id);
...
CREATE TABLE
    p_contact (
        ...
        -- 好友状态: 1请求好友, 2已好友, 3拒绝好友, 4已删好友
        is_top SMALLINT NOT NULL DEFAULT 0,
        -- 是否置顶, 0否, 1是
        is_black SMALLINT NOT NULL DEFAULT 0,
        -- 是否为黑名单, 0否, 1是
        is_del SMALLINT NOT NULL DEFAULT 0,
        -- 否删除好友, 0否, 1是
        notice_enable SMALLINT NOT NULL DEFAULT 0,
        -- 是否有消息提醒, 0否, 1是
        created_on BIGINT NOT NULL DEFAULT 0,
        modified_on BIGINT NOT NULL DEFAULT 0,
        deleted_on BIGINT NOT NULL DEFAULT 0
...
CREATE INDEX
    idx_contact_user_friend_status ON p_contact USING btree (user_id, friend_id, status);
...
CREATE TABLE
    p_contact_group (
        ...
        -- 分组名称
        is_del SMALLINT NOT NULL DEFAULT 1,
        -- 是否删除, 0否, 1是
        created_on BIGINT NOT NULL DEFAULT 0,
        modified_on BIGINT NOT NULL DEFAULT 0,
        deleted_on BIGINT NOT NULL DEFAULT 0
...
CREATE INDEX
    idx_wallet_recharge_user_id ON p_wallet_recharge USING btree (user_id);

CREATE INDEX
    idx_wallet_recharge_trade_no ON p_wallet_recharge USING btree (trade_no);

CREATE INDEX
    idx_wallet_recharge_trade_status ON p_wallet_recharge USING btree (trade_status);
...
CREATE INDEX
    idx_wallet_statement_user_id ON p_wallet_statement USING btree (user_id);
